...
public class MathStatement {
	...
	public boolean isAllNum() {
		for(Map.Entry<MathStatement, Boolean> en:subs.entrySet()) {
			if(!(en.getKey() instanceof Number))return false;
		}
		return true;
	}
	
	public static MathStatement format(String s) throws MathException {
		if(s.isEmpty())throw new MathException("Empty Statement", s, 0);
		...
		if(ms.subs.size()==1&&ms.subs.containsValue(true))return (MathStatement) ms.subs.keySet().toArray()[0];
		if(ms.isAllNum())ms=new Number(ms.calc(new HashMap<>()));
